	...
	public class PlotChannelRational : PlotChannelConsecutive
	{
		...
		protected override bool InternalHitTest(MouseEventArgs e)
		{
			...
			if (this.Markers.Visible)
			{
				...
				if (xAxis != null && yAxis != null)
				{
					...
					{
						PlotDataPointRational plotDataPointRational = this[num];
						Point point = base.GetPoint(plotDataPointRational.X, plotDataPointRational.Y);
						int num2 = (!this.DrawCustomDataPointAttributes) ? this.Markers.Size : plotDataPointRational.Marker.Size;
		...
		private void RationalInterpolation(int n, double x, out double y, out double dy)
		{
			...
			while (true)
			{
				if (num3 < n)
				{
					double num4 = Math.Abs(x - this.GetX(num3));
					if (num4 != 0.0)
					{
						if (num4 < num2)
						{
						...
						this.SetC(num3, this.GetY(num3));
						this.SetD(num3, this.GetY(num3) + 1E-25);
						num3++;
						continue;
					}
					break;
				...
				for (int i = 1; i < n; i++)
				{
					...
					while (num5 < n - i)
					{
						double num6 = this.GetC(num5 + 1) - this.GetD(num5);
						double num4 = this.GetX(num5 + i) - x;
						double num7 = (this.GetX(num5) - x) * this.GetD(num5) / num4;
						double num8 = num7 - this.GetC(num5 + 1);
						if (num8 != 0.0)
						{
							num8 = num6 / num8;
							this.SetD(num5, this.GetC(num5 + 1) * num8);
							this.SetC(num5, num7 * num8);
							num5++;
							continue;
						}
		...
		protected override void DrawMarkers(PaintArgs p, PlotXAxis xAxis, PlotYAxis yAxis, PlotMarker markers)
		{
			if (markers.Visible && this.IndexDrawStart != -1 && this.IndexDrawStop != -1)
			{
				...
				for (int i = this.IndexDrawStart; i <= this.IndexDrawStop; i++)
				{
					...
					{
						int num2 = xAxis.ScaleDisplay.ValueToPixels(plotDataPointRational.X);
						int num3 = yAxis.ScaleDisplay.ValueToPixels(plotDataPointRational.Y);
						if (this.DrawCustomDataPointAttributes)
						{
							if (base.XYSwapped)
							{
								((IPlotMarker)plotDataPointRational.Marker).Draw(p, num3, num2);
							}
							else
							{
								((IPlotMarker)plotDataPointRational.Marker).Draw(p, num2, num3);
							}
						}
